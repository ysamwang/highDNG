findGraphSum.test <- function(Y, maxInDegree = 3, degree = 3, B  = NULL, cutoff = 0) {
  
  .calcTau <- function(k, pa, ch) {
    abs(mean(pa^(k - 1) * ch) * mean(pa^2) - mean(pa^k) * mean(pa * ch))
  }
  
  getTau.Sum <- function(i, j, degree, lastRoot, conditionSet, maxInDegree, Y){
    # if there are no nodes in conditionSet, then just calculate tau on the raw data
    p <- dim(Y)[2]
    if(length(conditionSet) == 0){
      return(list(testStat = sum(sapply(setdiff(j, i), function(j){.calcTau(degree, Y[, i], Y[, j])})),
      ...
      for(z in 1:dim(conditionSubSet)[1]){
        ...
        singleSetStat <- min(singleSetStat, sum(tempStats))
  ...
  while(length(unordered) > 1){
    ...
    output <- foreach(i = unordered) %dopar%{
      ...
      getTau.Sum(i, j = unordered, degree = degree, lastRoot = ordered[length(ordered)],
